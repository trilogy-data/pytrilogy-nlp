...
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "log_data")
...
def log_prompt_info(prompt: TemplatedPromptCase, session_uuid: uuid.UUID):
    ...
    with open(os.path.join(DATA_DIR, str(session_uuid), prompt_hash + ".json"), "w") as f:
        print("printing to...{}".format(os.path.join(DATA_DIR, str(session_uuid), prompt_hash + ".json")))
        json.dump(data, f)
    
def run_prompt(prompt: TemplatedPromptCase, debug: bool = False, log_info=True, session_uuid: uuid.UUID=None) -> list[dict | list]:
    if not session_uuid:
        session_uuid = uuid.uuid4()
    if not os.path.exists(os.path.join(DATA_DIR, str(session_uuid))):
