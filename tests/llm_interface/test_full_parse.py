...
def test_ir_parsing():
    ...
    assert (
        str(having.conditional)
        == "local.total_return_amount<local.total_return_amount> > local.1.2_times_avg_return_per_store<store_returns.store.id>"
    ), str(having)
...
OUTPUT_TWO = {
    ...
    "action_input": {
        ...
        "filtering": {
            "root": {
                "values": [
                    {
                        "operator": "=",
                        "left": {"name": "store_returns.return_date.year"},
                        "right": {"value": "2000", "type": "integer"},
                    },
                    {
                        "operator": "=",
                        "left": {"name": "store_returns.store.state"},
                        "right": {"value": "TN", "type": "string"},
                    },
                    {
                        "operator": ">",
                        "left": {"name": "total_returns"},
                        "right": {
                            "name": "avg_returns_by_store",
                            "calculation": {
                                "operator": "MULTIPLY",
        ...
        "order": [{"column_name": "store_returns.customer.text_id", "order": "asc"}],
        "limit": 100,
    },
