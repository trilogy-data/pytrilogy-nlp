...
def test_age_class_query_resolution(normalized_engine: Executor, test_env):
    ...
    executor.environment = test_env

    for _ in range(3):
        query = nlp_engine.generate_query(
            text="What was the survival rate (ratio) by class? order by the class id, ascending",
            env=test_env,
        )
        ...
        for idx, row in enumerate(results):
            assert all(
                [y in row for y in expected[idx]]
            ), f"Expected {expected[idx]} but got {row}"
...
def test_filtering(normalized_engine: Executor, test_env):
    ...
    query = nlp_engine.generate_query(
        text="How many survivors in each cabin in 1st class?", env=test_env
    )
